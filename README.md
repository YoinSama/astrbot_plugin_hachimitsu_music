...
- **随机来一首**：`/哈基米` 从你勾选的随机池里抽（可选项 = 总榜前 N 名 + 全部风格标签），默认总榜 + 五个风格池各占一份，概率可自定义
- **随机池自己配**：总榜前 N 名和榜单里出现过的**全部风格标签**都能勾选，可以多选、至少留一个；概率默认按池子数**均分**（选 5 个就各 20%）
- **某个池想多出就手动固定它**：把池子的概率设成手动并填数值，没设的池子自动均分剩下的 —— 5 个池里把 A 固定成 50%，其余四个自动变 12.5%。单池最低 1%、总和不超 100%，填超了自动压回可填的最大值并说明原因（概率支持 1 位小数）
...
- **可视化配置**：音质、发送档位、随机池与概率、限流参数改完即时生效
- **配额管理**：查看与一键重置群 / 用户配额
...
| 指令 | 行为 | 权限 |
| --- | --- | --- |
| `/哈基米` | 随机一首（从配置的随机池里抽，各池概率可自定义） | 所有人 |
...
| 配置 | 默认 | 说明 |
| --- | --- | --- |
| `pools` | `总榜` + 五个风格池 | 参与随机的池子，可多选，至少留一个 |
| `pool_weights` | 空 | 手动指定个别池子的概率（选填），见下 |
| `top_n` | `1000` | 勾选了「总榜」时才生效：只从榜单前 N 名里抽 |
| `rank_refresh_hours` | `24` | 榜单刷新周期（小时） |

**随机池**：可选项 = 「总榜（前 N 名）」+ 榜单里出现过的全部风格标签（预设的五个是
`曼波好听～` / `冰🧊！` / `哈基周金曲` / `原教旨主义` / `婉约派`，其余标签随榜单动态增加）。
概率按**选中的池子数均分**：选 5 个就是每个 20%。

**手动指定概率**：在 `pool_weights` 里只填你想固定的池子，例如 `{"曼波好听～": 50}`，
没填的池子自动均分剩下的（上例其余四个各 12.5%）。单池最低 1%、总和不超 100%，
填超了会自动压回当前可填的最大值。全角标签名手输容易错，**推荐在插件控制台页里勾选**。
...
### 已完成
- 更多的预设风格池（v1.3.0：不再限于预设的五个，榜单里出现过的风格标签全部可选，且每个池的概率可手动指定）
...
打开插件控制台页的「常用配置 → 随机池」：勾选要参与随机的池子，需要固定某个池的概率就点它右边的
**✎** 改成手动（点 **×** 改回自动），保存后即时生效，不用重启。

旧的 `top_weight` / `style_weight` 两项已移除，升级时插件会把它们按原比例换算成一次池子选择。
