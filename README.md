...
| 配置 | 默认 | 说明 |
| --- | --- | --- |
| `user_cooldown_seconds` | 30 | 同一用户两次点歌的最短间隔（秒） |
| `group_cooldown_seconds` | 10 | 同一群两次点歌的最短间隔（秒） |
| `user_daily_limit` | 20 | 单个用户每天最多点歌数量（首），每天 0 点重置 |
| `group_daily_limit` | 100 | 单个群每天最多点歌数量（首），每天 0 点重置 |
| `global_per_minute` | 10 | 所有群合计每分钟最多处理数量（首/分钟） |
| `max_concurrency` | 2 | 同时处理的点歌任务数上限（个），建议 2~3 |
| `queue_max` | 20 | 等待队列最大长度（个），超出后提示稍后再试 |
...
### 未完成需求
- 作品长度限制作为可获取范围的配置项（例如10分钟及以上的作品踢出随机池）
- 稳定和限流配置的哨兵值配置（例如某项配置为`-1`，该限制不生效）
